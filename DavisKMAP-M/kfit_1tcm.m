function [k, cfit] = kfit_1tcm(ct, cp, scant, k0, opt, wt, cwb)
%--------------------------------------------------------------------------
% Fit time activity curves using the two-tissue compartmental model (1TCM)
...
% - k0: Initial kinetic parameters [vb, K1, k2, time_delay].
...
%
% Outputs:
% - k: Fitted kinetic parameters [vb, K1, k2, time_delay].
% - cfit: Fitted TAC curve.
%
% This function serves as a wrapper around the compiled MEX file 
% 'kfit_1tcm_mex_omp.mexa64', which performs the actual optimization using
...
% Initialize kinetic parameters [vb K1 k2 time_delay]
if isempty(k0)
    k0 = [0.05 0.1 0.1 0]; % Default initial parameters
end
if isvector(k0)
...
if isempty(opt.LowerBound)
    lb = [0, 0, 0, 0]; % Default lower bounds
elseif length(opt.LowerBound) == 4
    lb = opt.LowerBound;
else
    error('Size of lower bounds should be 4.');
end

% Set upper bounds for the parameters
if isempty(opt.UpperBound)
    ub = [1.0, 2.0, 2.0, 20]; % Default upper bounds
elseif length(opt.UpperBound) == 4
    ub = opt.UpperBound;
else
    error('Size of upper bounds should be 4.');
end

% Set parameter sensitivity (which parameters to fit)
if isempty(opt.PrmSens)
    ps = [1, 1, 1, 1]; % Default to fitting all parameters
elseif length(opt.PrmSens) == 4
    ps = opt.PrmSens;
else
    error('Size of active label should be 4.');
end
...
[k, cfit] = kfit_1tcm_mex_omp(ct, wt, scant, cp, cwb, dk, kinit, lb, ub, ...
                         ps, maxit, td);
end
