function [k, cfit, ctis] = kfit_liverNew(ct, cp, scant, k0, opt, wt, cwb)
%--------------------------------------------------------------------------
...
% initial kinetic parameters  [va K1 vd k3 k4 ka fa]'
if isempty(k0)
...
[k,cfit] = kfit_liverNew_mex_omp(ct, wt, scant, cp, cwb, dk, kinit, lb, ub, ...
                         ps, maxit, td);
% IRF                     
if nargout>2
    u0 = zeros(size(cp));
    u0(1) = 1;
    ctis = ktac_liverNew(k, u0, scant, opt, cwb);
end
