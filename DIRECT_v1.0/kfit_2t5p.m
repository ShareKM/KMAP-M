...
%--------------------------------------------------------------------------
% Fit time activity curves using the two-tissue compartmental model (2T5P)
% with five parameters. The Gaussian noise model is used for the fitting.
%
% Inputs:
% - ct: Measured time-activity curve (TAC) data.
% - cp: Plasma input function.
% - scant: Scan time data, provided in seconds.
% - k0: Initial kinetic parameters [vb, K1, k2, k3, k4].
...
%
% Outputs:
% - k: Fitted kinetic parameters [vb, K1, k2, k3, k4].
% - cfit: Fitted TAC curve.
%
% This function serves as a wrapper around the compiled MEX file 
% 'kfit_2t5p_mex_omp.mexa64', which performs the actual optimization using
...
% Initialize kinetic parameters [vb K1 k2 k3 k4]
if isempty(k0)
    k0 = [0.05 0.1 0.1 0.1 0.01]; % Default initial parameters
end
if isvector(k0)
...
if isempty(opt.LowerBound)
    lb = [0, 0, 0, 0, 0]; % Default lower bounds
elseif length(opt.LowerBound) == 5
    lb = opt.LowerBound;
else
    error('Size of lower bounds should be 5.');
end

% Set upper bounds for the parameters
if isempty(opt.UpperBound)
    ub = [1.0, 2.0, 2.0, 1.0, 1.0]; % Default upper bounds
elseif length(opt.UpperBound) == 5
    ub = opt.UpperBound;
else
    error('Size of upper bounds should be 5.');
end

% Set parameter sensitivity (which parameters to fit)
if isempty(opt.PrmSens)
    ps = [1, 1, 1, 1, 1]; % Default to fitting all parameters
elseif length(opt.PrmSens) == 5
    ps = opt.PrmSens;
else
    error('Size of active label should be 5.');
end
...
[k, cfit] = kfit_2t5p_mex_omp(ct, wt, scant, cp, cwb, dk, kinit, lb, ub, ...
                         ps, maxit, td);
end
