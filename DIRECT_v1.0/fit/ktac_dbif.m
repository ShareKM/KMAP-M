function [ct, st] = ktac_dbif(k, cp, scant, opt, cwb)
%--------------------------------------------------------------------------
...
% adapt the input vector to [vb K1 k2 k3 k4 Ka fa kg]'
prm = zeros(8, size(k,2));
if size(k,1)==3     % one-tissue
    prm(1:3,:) = k;
elseif size(k,1)==5 % two-tissue
    prm(1:size(k,1),:) = k;
elseif size(k,1)==8 % two-tissue
    prm(1:size(k,1),:) = k;
end
...
if nargout==2 & size(prm,2)==1
    [ct, st] = ktac_dbif_mex(prm, scant, cp, cwb, dk, td);
else
    ct = ktac_dbif_mex(prm, scant, cp, cwb, dk, td);
    st = [];
end
