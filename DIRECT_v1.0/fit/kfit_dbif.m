function [k, cfit, ctis] = kfit_dbif(ct, cp, scant, k0, opt, wt, cwb)
%--------------------------------------------------------------------------
% Fit time activity curves using the two-tissue compartmental model with 
% five kinetic parameters plus dispersion and fa. Gaussian noise 
% model is used.
%
% Guobao Wang @ 11-06-2017
%
%--------------------------------------------------------------------------
...
% initial kinetic parameters  [va K1 k2 k3 k4 fa Kg tg'
if isempty(k0)
    k0 = [0.05 0.1 0.1 0.1 0.01 0.1 1 1];
end
if isvector(k0)
...
elseif length(opt.LowerBound)==8
    ...
    error('Size of lower bounds is 8');
end
if isempty(opt.UpperBound)
    ub = [1.0, 10, 5.0, 1.0, 0.1, 20, 1 20];
elseif length(opt.UpperBound)==8
    ub = opt.UpperBound;
else
    error('Size of upper bounds is 8');
end
...
elseif length(opt.PrmSens)==8
    ...
    error('Size of active label is 8');
end
...
[k,cfit] = kfit_dbif_mex_omp(ct, wt, scant, cp, cwb, dk, kinit, lb, ub, ...
                         ps, maxit, td);
% IRF                     
if nargout>2
    u0 = zeros(size(cp));
    u0(1) = 1;
    ctis = ktac_dbif(k, u0, scant, opt, cwb);
end
