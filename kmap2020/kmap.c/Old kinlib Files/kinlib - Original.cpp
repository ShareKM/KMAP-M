...
// calculate the time acitivity curve using the two-tissue kinetic model. 
{
   ...
   for (j=0; j<num_vox; j++) {
      ...
      // average in frame duration
      frame(scant, td, c_t, num_frm, 1, ct+j*num_frm);
      frame(scant, td, c_f, num_frm, 1, cf+j*num_frm);
      frame(scant, td, c_b, num_frm, 1, cb+j*num_frm);
   }
...
// calculate the time acitivity curve using the two-tissue kinetic model. 
{
   ...
   for (j=0; j<num_vox; j++) {
      ...
      for (i=0; i<num_time; i++) {
         c_f[i] = f1*c_a1[i] + f2*c_a2[i];
         c_b[i] = b1*c_a1[i] + b2*c_a2[i];
         c_t[i] = (c_f[i] + c_b[i]) + va*cp[i] + vb*wb[i];
      }
...
//------------------------------------------------------------------------------
// calculate the time activity curves in second and the sensitivity functions 
//	for the two-tissue compartment model. The sensitivity function of vd (=k1/k2)
// is calculated instead of calculting for k2, 
{
   int      i, j;
   double   vb, k1, k2, k3, k4, vd;
   double   d, a1, a2, f1, f2, b1, b2, k234, tmp;
   ...
   for (j=0; j<num_vox; j++) {
      ...
      if (psens[1]==1) { // wrt k1 (not completed)
         for (i=0; i<num_time; i++)
            s_t[i] = (1-vb) * ( f1*c_a1[i] + f2*c_a2[i]);
         s_t += num_time;
      }
      ...
      if (psens[2]==1) { // wrt vd
         for (i=0; i<num_time; i++)
            s_t[i] = (1-vb) * k1/vd/vd * ( f1*c_a1[i] + f2*c_a2[i] );
         s_t += num_time;
      }
      if (psens[1]==1) { // wrt k1 (completed)
         if (psens[2]==1)
            s_t -= num_time;
         s_t -= num_time;
         for (i=0; i<num_time; i++)
            s_t[i] -= (1-vb) / vd  * ( f1*c_a1[i] + f2*c_a2[i]);
         s_t += num_time;
         if (psens[2]==1)
            s_t += num_time;
      }
...
// is calculated instead of calculting for k2, 
{
   ...
   for (j=0; j<num_vox; j++) {
      ...
      if (psens[2]==1) { // wrt vd
         for (i=0; i<num_time; i++)
            s_t[i] = (1-vb) * k1*k3/ki/ki * ( f1*c_a1[i] + f2*c_a2[i] );
         s_t += num_time;
      }
      if (psens[1]==1) { // wrt k1 (completed)
         if (psens[2]==1)
         ...
         s_t -= num_time;
         for (i=0; i<num_time; i++)
            s_t[i] -= (1-vb)* k3/ki  * ( f1*c_a1[i] + f2*c_a2[i]);
         s_t += num_time;
         if (psens[2]==1)
            s_t += num_time;
      }
		if (psens[3]==1) { // wrt k3
		   f1 = (1-vb) * 1/d * ( k1/ki*(a1-k3-k4) + a2 );
		   f2 = (1-vb) * 1/d * ( k1/ki*(k3+k4-a2) - a1 );
			for (i=0; i<num_time; i++)
				s_t[i] = f1*c_a1[i] + f2*c_a2[i];
			s_t += num_time;
		}
...
void lema_psn(double *w, double *y, double *f, int num_y, double *p, int num_p, 
              ...
              double *plb, double *pub, int *psens, int maxit)
//------------------------------------------------------------------------------
// The Levenberg-Marquardt algorithm to minimize the negative Poisson likelihood
// subject to linear bounds.
{
   int      i, j, k, it, n;
   double   mu, v, tau, rho, maxh;
...
void lema_png(double *w, double *y, double *f, double *wg, double *yg, int num_y,  
              ...
              double *plb, double *pub, int *psens, int maxit)
//------------------------------------------------------------------------------
// The Levenberg-Marquardt algorithm to minimize the compound form of negative
// Poisson and Gaussian likelihood subject to linear bounds.
{
   int      i, j, k, it, n;
   double   mu, v, tau, rho, maxh;
   int      num_par;
   ...
   while (it<maxit) {
      ...
      if (rho>0.0)
         for (j=0; j<num_par; j++) {
            g[j] = 0.0;
            for (i=0; i<num_y; i++) {
               h1 = w[i] * ( 1.0 - y[i]/f[i] ) + wg[i] * ( f[i] - yg[i] );
               g[j] +=  h1 * J[i+j*num_y];
            }
            for (k=0; k<num_par; k++) {
               H[j+k*num_par] = 0.0;
               for (i=0; i<num_y; i++) {
                  h2 = w[i] * y[i]/(f[i]*f[i]) + wg[i];
                  H[j+k*num_par] += h2 * J[i+j*num_y] * J[i+k*num_y];
               }
            }
         }
      ...
      for (i=0; i<num_y; i++) {
         Fnew += w[i] * ( f[i] - y[i]*log(f[i]) );
         Fnew += 0.5 * wg[i] * ( f[i] - yg[i] ) * ( f[i] - yg[i] );
      }
      rho = (Fnew-F) / L;
...
{
   int      i, j;
   double   vb, K1, k2, k3, k4, fv, Kd;
   double   d, a1, a2, f1, f2, b1, b2, k234, tmp;
   double   *c_pv, *c_a1, *c_a2, *c_f, *c_b, *c_t, *cin;
   int      num_time;
   ...
   c_b  = (double*) malloc(sizeof(double)*num_time);
   c_t  = (double*) malloc(sizeof(double)*num_time);
   cin  = (double*) malloc(sizeof(double)*num_time);	
   ...
   for (j=0; j<num_vox; j++) {
      ...
      // dispersion
      tmp = K1*fv + dk;
      kconv_exp(1.0, tmp, ca, num_time, td, c_pv);
      for (i=0; i<num_time; i++) {
         cin[i] = fv * Kd*c_pv[i] + (1-fv)*ca[i];
      }
      
      // exp components
      tmp = a1 + dk;
      kconv_exp(1.0, tmp, cin, num_time, td, c_a1);
      tmp = a2 + dk;
      kconv_exp(1.0, tmp, cin, num_time, td, c_a2);
      ...
      for (i=0; i<num_time; i++) {
         c_f[i] = f1*c_a1[i] + f2*c_a2[i];
         c_b[i] = b1*c_a1[i] + b2*c_a2[i];
         c_t[i] = (1-vb) * (c_f[i] + c_b[i]) + vb*cin[i];
      }
...
//	for the two-tissue compartment model.
{
   ...
   double   vb, K1, k2, k3, k4, fv, Kd;
   double   d, a1, a2, f1, f2, b1, b2, k234, tmp;
   double   *c_pv, *c_a1, *c_a2, *c_f, *c_b, *c_t, *s_t, *cin, *c_vv;
   int      num_time;
   int      num_par;
   ...
   s_t  = (double*) malloc(sizeof(double)*num_time*num_par);
   cin  = (double*) malloc(sizeof(double)*num_time);
   c_vv = (double*) malloc(sizeof(double)*num_time);	
   ...
   for (j=0; j<num_vox; j++) {
      ...
      if (psens[1]==1 || psens[5]==1) { 
         tmp = K1*fv + dk;
         kconv_exp(1.0, tmp, c_pv, num_time, td, c_vv);
      }
      if (psens[1]==1) { // wrt K1
         for (i=0; i<num_time; i++)
             cin[i] = - K1*fv*fv*c_vv[i] + cin[i];
         tmp = a1 + dk;
         kconv_exp(1.0, tmp, cin, num_time, td, c_a1);
         tmp = a2 + dk;
         kconv_exp(1.0, tmp, cin, num_time, td, c_a2);
         ...
         for (i=0; i<num_time; i++)
            s_t[i] = (1-vb) * ( f1*c_a1[i] + f2*c_a2[i]) - vb*fv*fv*c_vv[i];
         s_t += num_time;
      }
      if (psens[2]==1 || psens[3]==1) { 
         tmp = a1 + dk;
	 ...
	 s_t += num_time;
      ...
      if (psens[5]==1) { // wrt fv
         for (i=0; i<num_time; i++)
             cin[i] = - K1*fv*K1*c_vv[i] + K1*(Kd*c_pv[i]-ca[i]);
         tmp = a1 + dk;
         kconv_exp(1.0, tmp, cin, num_time, td, c_a1);
         tmp = a2 + dk;
         kconv_exp(1.0, tmp, cin, num_time, td, c_a2);
         ...
         for (i=0; i<num_time; i++) {
            s_t[i] = (1-vb)*( f1*c_a1[i] + f2*c_a2[i] ) + vb*(Kd*c_pv[i]-ca[i]) - vb*fv*K1*c_vv[i];
         }
         s_t += num_time;
      }
      if (psens[6]==1) { // wrt Kd
         for (i=0; i<num_time; i++)
             cin[i] = - K1*fv*Kd*c_pv[i];
         tmp = a1 + dk;
         kconv_exp(1.0, tmp, cin, num_time, td, c_a1);
         tmp = a2 + dk;
         kconv_exp(1.0, tmp, cin, num_time, td, c_a2);
         for (i=0; i<num_time; i++) {
            s_t[i] = (1-vb) * ( f1*c_a1[i] + f2*c_a2[i] ) + vb*fv*Kd*c_pv[i];
         }
         s_t += num_time;
      }  
...
// calculate the time acitivity curve using the two-tissue kinetic model. 
{
   ...
   for (j=0; j<num_vox; j++) {
      ...
      for (i=0; i<num_time; i++) {
         c_f[i] = f1*c_a1[i] + f2*c_a2[i];
         c_b[i] = b1*c_a1[i] + b2*c_a2[i];
         c_t[i] = (1-vb-ve) * (c_f[i] + c_b[i]) + ve*c_f[i] + vb*wb[i];
      }
...
//	for the two-tissue compartment model.
{
   ...
   for (j=0; j<num_vox; j++) {
      ...
      if (psens[2]==1 || psens[3]==1) { 
         f1 = 1/d*( (1-vb)*(k4-a1) + (1-vb-ve)*k3 );
         f2 = 1/d*( (1-vb)*(a2-k4) - (1-vb-ve)*k3 );
      }
      if (psens[2]==1) { // wrt k1
         for (i=0; i<num_time; i++)
            s_t[i] = f1*c_a1[i] + f2*c_a2[i];
      ...
      if (psens[4]==1 || psens[5]==1) { 
         f1 = 1/d*( (1-vb)*a1 + (1-vb-ve)*(a2-k3-k4) );
         f2 = 1/d*(-(1-vb)*a2 + (1-vb-ve)*(k3+k4-a1) );
      }
      if (psens[4]==1) { // wrt k3
         for (i=0; i<num_time; i++)
            s_t[i] = f1*c_a1[i] + f2*c_a2[i];
